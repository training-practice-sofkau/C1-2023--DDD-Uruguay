...
import { CompanyDomainEntityBase, FeeDomainEntityBase, InvoiceDomainEntityBase } from '../../../domain/entities/';
    ...
    implements IUseCase<Command, Response>
{
    ...
    private async executeCommand(
        ...
        const invoice = await this.invoiceAggregateRoot.getInvoice(command.invoiceId);
        this.validateAggregate(invoice);
        return invoice;
    }

    private validateAggregate(
        aggregate: IInvoiceDomainEntity
    ): void {
        const {
            invoiceId,
            status,
